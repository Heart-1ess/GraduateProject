...
LOCAL_ROBERTA_PATH = os.path.join(ROOT_DIR, 'model/hpm_adm_dual/bert-base')

class BERTDualChannelModel(nn.Module):
    def __init__(self, pretrained_model=LOCAL_ROBERTA_PATH, num_heads=4, embed_dim=768, hidden_dim=256, emotion_dim=28):
        super(BERTDualChannelModel, self).__init__()
        
        # 加载预训练的BERT模型
        self.roberta = BertModel.from_pretrained(pretrained_model)
